functions {
  ...
  /*
     System of equations to find shape parameters of the GLD that yield bounds
  
     See equations 13 and 14 of 
     https://mpra.ub.uni-muenchen.de/43333/3/MPRA_paper_43333.pdf
  
     @param free vector of size two pre-parameters of chi and xi
     @param theta vector of size zero that is unused
     @param x_r real array of size four containing:
       {lower_bound, upper_bound, median, IQR}
     @param x_i integer array of size zero that is unused
     @return vector of two residuals
   */
  vector equations3(vector free, vector theta, data real[] x_r, data int[] x_i) {
    real chi = tanh(free[1]);
  ...
  vector GLD_solver_bounded(data row_vector bounds, data real m, data real r) {
    ...
    vector[2] free = algebra_solver(equations3,
                                    ...
                                    rep_vector(0, 0), 
                                    {bounds[1], bounds[2], m, r}, x_i);
    real chi = tanh(free[1]);
    real ub = 0.5 * fmin(1 + chi, 1 - chi);
    real xi = inv_logit(free[2]) * ub - machine_precision();
    return [chi, xi]';
  }
